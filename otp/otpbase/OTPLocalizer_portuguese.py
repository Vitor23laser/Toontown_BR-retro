...
WhisperUnavailable = "Aquele jogador não está disponivel para cochicho."
...
UnpaidNoParentPasswordWarning = 'Este é para o recurso Chat Rápido Plus, qual permite usários para bater um papo por digitando palavras encontradas no Dicionário Chat Rápido Plus da Disney. Para ativar este recurso ou para aprender mais sobre ele, saia de Toontown e clique em "Opções da conta" na página da web de Toontown.'
PaidNoParentPasswordWarningSet = "Atualizar Opções de Chat"
...
NoSecretChatWarning = 'Para conversar com um amigo, o recurso Amigos secretos deve estar ativado. As crianças precisam que seus pais visitem o site de Toontown para conhecer o recurso Amigos secretos.'
RestrictedSecretChatWarning = 'Para pegar ou digitar um Código Secreto, faça login com a conta de pais. Você pode desativar esta solicitação alterando as suas opções de Amigos secretos.'
...
NoSecretChatAtAll = 'O recurso Chat de Amigos Secretos permite amigos da vida-real para conversar abertamente com uns aos outros por meio de um código secreto que deve ser compartilhado fora do jogo\n\nPara ativar este recurso ou para aprender mais sobre ele, saia de Toontown e clique em "Opções da conta" na página da web de Toontown.'
NoSecretChatAtAllAndNoWhitelistTitle = "Botão de Chat"
# not sure what this should do in the new world order
NoSecretChatAtAllAndNoWhitelist = 'Você pode usar o botão azul de chat para se comunicar com outros Toons usando o recurso Chat Rápido Plus ou o recurso Chat de Amigos Secretos.\n\nO recurso Chat rápido Plus é uma forma de tipo de chat que permitem usuários para comunicarem por usar o dicionário Disney Chat Rápido Plus.\n\nO recurso Chat de Amigos Secretos é uma forma de tipo de chat que permite amigos da vida-real para conversar abertamente com uns aos outros por meio de um código secreto que deve ser compartilhado fora do jogo.\n\nPara ativar estes recursos ou para aprender mais sobre ele, saia de Toontown e clique em "Opções da conta" na página da web de Toontown.'
...
ActivateChat = """O recurso Amigos Secretos permite que um membro converse com outro membro apenas por meio de um código secreto que deve ser comunicado fora do jogo. O recurso Amigos Secretos não é moderado ou supervisionado.
   ...
   Isso oferece o mais alto nível de controle.
      ...
      \x01shadow\x1O sistema Amigos Secretos irrestritos\x02 - Depois de habilitado com a senha de pais,
...
SecretFriendsInfoPanelText = [""" O recurso Chat de Amigos Secretos O recurso Chat de Amigos Secretos ativa um membro para conversar diretamente com outro no Toontown On-line da Disney (o "Serviço") depois que os membros estabelecerem uma conexão de Amigos secretos. Quando o seu filho tentar usar o recurso Chat de Amigos Secretos, solicitaremos que você insira a sua Senha da conta de pais para indicar seu consentimento para que a criança use o recurso. Esta é uma descrição detalhada do processo de criação de uma conexão de Chat de Amigos Secretos entre os membros fictícios chamados "Sandra" e "Marcos". 1. O responsável por Sandra e o responsável por Marcos ativam o recurso Chat de Amigos Secretos inserindo suas respectivas Senhas conta de pais (a) nas áreas de Opções da conta do Serviço ou (b) quando for solicitado no jogo, em uma janela pop-up de Controles disponíveis aos pais. 2. Sandra pede um Código Secreto (descrito abaixo) no Serviço. """,""" 3. O Código Secreto de Sandra é comunicado a Marcos fora do Serviço. (O Código Secreto de Sandra pode ser comunicado a Marcos diretamente por Sandra ou indiretamente, se Sandra revelar o Código Secreto a outra pessoa.) 4. Marcos envia o Código Secreto de Sandra ao Serviço dentro de 48 horas a partir da hora em que Sandra solicitou o Código Secreto ao Serviço. 5. Em seguida, o Serviço notifica Marcos de que Sandra tornou-se sua Amiga secreta. Da mesma forma, o Serviço notifica Sandra de que Marcos tornou-se seu Amigo secreto. 6. Sandra e Marcos podem agora bater um papo aberto diretamente um com o outro até um deles escolher cancelar o seu relacionamento como Amigo secreto, ou até que o recurso Chat de Amigos Secretos seja desativado para Sandra ou Marcos por um dos responsáveis. Então, a conexão de Amigos secretos pode ser desativada a qualquer momento: (a) por um membro, que remove o Amigo secreto de sua lista de amigos (conforme descrito no Serviço), ou (b) pelo responsável pelo membro, que desativa o recurso Abrir chat com """,""" Amigos secretos na área Opções da conta do Serviço, seguindo as etapas definidas no recurso. O Código Secreto é um código aleatório, gerado por computador, que é atribuído a um membro específico. O Código Secreto precisa ser usado para ativar a conexão de Amigo secreto dentro de 48 horas a partir da hora em que o membro solicitou o Código Secreto; caso contrário, o Código Secreto expirará e não poderá ser usado. Além disso, só se pode usar um único Código Secreto para estabelecer uma conexão de Amigo secreto. Para fazer conexões adicionais de Amigos secretos, o membro precisará solicitar mais códigos de Amigo secretos, um para cada Amigo secreto que quiser incluir. As Amizades secretas não podem ser transferidas. Por exemplo, se Sandra se tornar Amiga secreta de Marcos, e Marcos se tornar Amigo secreto de Jéssica, Sandra não se tornará automaticamente Amiga secreta de Jéssica. Para que Sandra e Jéssica """,""" se tornem Amigas secretas, uma delas terá que solicitar um novo Código Secreto ao Serviço e comunicar à outra. Os Amigos secretos se comunicam entre si por meio de uma conversa interativa em formato livre. O conteúdo da conversa é inserido diretamente pelo membro participante e é processado pelo Serviço, cuja operação é realizada pelo Walt Disney Internet Group ("WDIG"), 500 S. Buena Vista St., Burbank, CA 91521-7691.  Embora aconselhamos os membros não trocarem informações pessoais como nome e sobrenome, e-mails, endereço postal ou números de telefone ao usarem o recurso Chat de Amigos Secretos, não podemos garantir que os membros seguirão a recomendação e que tais informações sejam preservadas. Embora o chat Amigos secretos seja automaticamente filtrado para maioria dos palavrões, Chat de Amigos Secretos pode ser moderado e a Disney reserva-se o direito de moderar qualquer parte do Serviço que a Disney, """,""" a seu exclusivo e absoluto critério, julgar necessário. No entanto, como Chat de Amigos Secretos nem sempre será moderado, se a conta de pais deixarem seus filhos usarem a conta com o recurso Chat de Amigos Secretos ativado no Serviço, aconselhamos que eles mesmos supervisionem os filhos durante a brincadeira. Ao ativar o recurso Chat de Amigos Secretos, a conta de pais reconhece que, apesar de haver alguns riscos inerentes ao recurso Chat de Amigos Secretos, a conta de pais foi informada de todos os riscos mencionados aqui, concordando em aceitá-los, seja previsível ou não. O WDIG não usa o conteúdo do chat Amigos secretos para nenhum fim que não seja a comunicação do conteúdo ao amigo secreto do membro, e não revela tal conteúdo a terceiros, exceto: (1) se exigido por lei; por exemplo, para cumprir uma ordem ou intimação judicial; (2) para fazer com que os Termos de Uso """,""" aplicáveis ao Serviço (que podem ser acessados na página principal do Serviço) sejam respeitados; ou (3) para proteger a segurança dos Membros do Serviço e o Serviço propriamente dito. Mediante solicitação ao WDIG, o responsável por uma criança-membro pode analisar e mandar apagar qualquer conteúdo do recurso de chat Amigos secretos fornecidos pela criança em questão, desde que tal conteúdo já não tenha sido excluído dos registros pelo WDIG. Obedecendo à Children's Online Privacy Protection Act, uma lei americana de proteção à privacidade on-line para as crianças, estamos proibidos de condicionar a participação da criança em qualquer tipo de atividade (inclusive o recurso Chat de Amigos Secretos) ao fornecimento, por parte da criança, de mais informações pessoais do que o estritamente necessário para que ela participe de tais atividades.  Além disso, conforme observado acima, reconhecemos o direito do responsável pela criança de não permitir que continuemos a deixar que a criança use o recurso Amigos secretos. Ao ativar o recurso Chat de Amigos Secretos, você reconhece que há alguns riscos inerentes ao chat aberto, no qual os membros podem conversar uns com os outros usando o recurso Chat de Amigos Secretos, sendo que você foi informado de todos os riscos mencionados aqui, concordando em aceitá-los, seja previsível ou não."""
]
...
FriendSecretNeedsParentLoginWarning = """Para conseguir ou digitar um Código Secreto, um dos seus pais ou responsáveis precisa fazer o login. Você pode desativar esta pergunta alterando suas opções de Amigos Secretos."""
FriendSecretNeedsPasswordWarning = """Para pegar ou digitar um Código Secreto, você deve inserir a Senha da conta de pais. Você pode desativar esta solicitação alterando as suas opções de Amigos verdadadeiros."""
...
FriendSecretIntro = "Se você estiver jogando Toontown Online da Disney com alguém que conhece no mundo real, poderá tornar-se Amigo secreto dessa pessoa. Você pode conversar com seus Amigos secretos usando o teclado. Os outros Toons não entenderão o que vocês estiverem falando.\n\nVocê pode conseguir isto obtendo um Código Secreto. Conte o Código Secreto só ao seu amigo, e a mais ninguém. Quando o seu amigo digitar o seu Código Secreto na tela, vocês dois serão Amigos secretos em Toontown!"
FriendSecretGetSecret = "Obter um Código Secreto"
FriendSecretEnterSecret = "Se você tiver um Código Secreto de alguém conhecido, digite-o aqui."
FriendSecretOK = lOK
FriendSecretEnter = "Inserir Código Secreto"
FriendSecretCancel = lCancel
FriendSecretGettingSecret = "Obtendo Código Secreto. . ."
FriendSecretGotSecret = "Este é o seu novo Código Secreto. Não deixe de anotá-lo em algum lugar!\n\nVocê só pode dar este Código Secreto a uma pessoa. Depois que alguém digitar o seu Código Secreto, ele não funcionará para nenhuma outra pessoa. Se você quiser dar um Código Secreto para mais de uma pessoa, obtenha outro.\n\nO Código Secreto só funcionará nos próximos dois dias. O seu amigo terá que digitá-lo antes que expire, caso contrário, não funcionará.\n\nO Código Secreto é:"
...
FriendSecretTimeOut = "Sinto muitos, segredo não estão funcionando agora."
...
FriendInviterNotOpen = "%s não tem um bate-papo aberto, use Código Secretos para fazer amigos"
...
SpeedChatStaticTextToontown = {
    ...
    1130 : "Vamos assumir um escritório de campo!",
...
CustomSCStrings = {
    ...
    11021 : "As decorações da festa de fim de ano são Toontastico!",
